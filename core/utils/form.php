...
class form
{
    ...
    {
        return $this->model->load($this->getData())->save();
    }
    ...
    private function renderFieldHtml(array $field): string
    {
        ...
        return match ($field['type']) {
            ...
            'file' => "<input type=\"file\" name=\"{$field['name']}" . ($field['multiple'] ? '[]' : '') . "\" id=\"{$field['id']}\" $attrs class=\"{inputClass} " . ($field['hasError'] ? '{inputErrorClass}' : '') . "\" " . ($field['multiple'] ? 'multiple' : '') . " " . (!isset($field['value']) ? $required : '') . "> " . (isset($field['value']) ?  (is_array($field['value']) ? count($field['value']) . ' files uploaded' : $field['value']) : '') . (isset($field['value']) ? '<input type="hidden" name="_' . $field['name'] . '" value="' .  implode(',', (array) $field['value']) . '">' . (is_array($field['value']) ? ('<p style="font-size:12px;margin-top: 2px;">' . implode('<br/>', $field['value']) . '</p>') : '') : ''),
